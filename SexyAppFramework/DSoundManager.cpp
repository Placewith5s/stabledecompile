...
bool DSoundManager::LoadWAVSound(unsigned int theSfxID, const SexyString& theFilename)
{		
	int aDataSize;
	...
	if (!sexystrcmp(aChunkType, _S("RIFF")) == 0)
		return false;
	p_fread(&aChunkSize, 4, 1, fp);

	p_fread(aChunkType, 1, 4, fp);
	if (!sexystrcmp(aChunkType, _S("WAVE")) == 0)
		return false;
	...
	while (!p_feof(fp))
	{
		...
		if (sexystrcmp(aChunkType, _S("fmt ")) == 0)
		{
			ushort aFormatTag;
			ulong aBytesPerSec;
			ushort aBlockAlign;			
		...
		else if (sexystrcmp(aChunkType, _S("dep ")) == 0)
		{
			SexyChar aStr[256];
			ushort aStrLen;
		...
		else if (sexystrcmp(aChunkType, _S("xor ")) == 0)
		{			
...
bool DSoundManager::LoadFModSound(unsigned int theSfxID, const SexyString& theFilename)
{
	if (!mHaveFMod)
		return false;
...
bool DSoundManager::LoadOGGSound(unsigned int theSfxID, const SexyString& theFilename)
{
	OggVorbis_File vf;
	int current_section;

	PFILE *aFile = p_fopen(theFilename.c_str(), _S("rb"));
	if (aFile==NULL)
		return false;
...
bool DSoundManager::LoadAUSound(unsigned int theSfxID, const SexyString& theFilename)
{
	PFILE* fp;
...
bool DSoundManager::LoadSound(unsigned int theSfxID, const SexyString& theFilename)
{
	...
	SexyString aFilename = theFilename;
	SexyString aCachedName;

	if ((aFilename.length() > 2) && (aFilename[0] != _S('\\')) && (aFilename[0] != _S('/')) &&
		(aFilename[1] != ':'))
	{
		// Not an absolute path
		aCachedName = GetAppDataFolder() + _S("cached\\") + aFilename + _S(".wav");
		if (LoadWAVSound(theSfxID, aCachedName))
			return true;
		MkDir(GetFileDir(aCachedName));
	}		

	if (LoadWAVSound(theSfxID, aFilename + _S(".wav")))
		return true;

	if (mHaveFMod)
	{
		if (LoadFModSound(theSfxID, aFilename + _S(".mp3")))
		{
			WriteWAV(theSfxID, aCachedName, aFilename + _S(".mp3"));
			return true;
		}
#ifndef USE_OGG_LIB
		if (LoadFModSound(theSfxID, aFilename + _S(".ogg")))
		{		
			WriteWAV(theSfxID, aCachedName, aFilename + _S(".ogg"));
			return true;
		}
#endif
	}
...
#ifdef USE_OGG_LIB
	if (LoadOGGSound(theSfxID, aFilename + _S(".ogg")))
	{		
		WriteWAV(theSfxID, aCachedName, aFilename + _S(".ogg"));
		return true;
	}
	...
	if (LoadAUSound(theSfxID, aFilename + _S(".au")))
	{
		WriteWAV(theSfxID, aCachedName, aFilename + _S(".au"));
		return true;
	}
...
int DSoundManager::LoadSound(const SexyString& theFilename)
{
	int i;
	for (i = 0; i < MAX_SOURCE_SOUNDS; i++)
...
bool DSoundManager::GetTheFileTime(const SexyString& theDepFile, FILETIME* theFileTime)
{	
...
bool DSoundManager::WriteWAV(unsigned int theSfxID, const SexyString& theFilename, const SexyString& theDepFile)
{
	...
	fwrite(_S("RIFF"), 1, 4, fp);	
	fwrite(&aChunkSize, 4, 1, fp);
	fwrite(_S("WAVE"), 1, 4, fp);
