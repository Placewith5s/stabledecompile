...
void LawnDialog::MouseWheel(int theDelta)
{
...
void ReanimationWidget::Draw(Graphics* g)
{
    if (mReanim)
    {
        Image* img = mApp && mApp->mBoard && mApp->mBoard->StageIsNight()? IMAGE_PLANTSHADOW2 : IMAGE_PLANTSHADOW;
        if (mReanim->mReanimationType == ReanimationType::REANIM_ZOMBIE_NEWSPAPER) 
    ...
    Dialog::BUTTONS_FOOTER)
{
    ...
    mMenuButton->Resize(635 - mX + mApp->mDDInterface->mWideScreenOffsetX, -10 - mY + mApp->mDDInterface->mWideScreenOffsetY, 163, 46);
